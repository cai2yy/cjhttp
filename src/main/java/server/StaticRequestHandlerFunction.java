...
public class StaticRequestHandlerFunction implements HandlerFunction {
	...
	public StaticRequestHandlerFunction() {
		this("/");
	}

	public StaticRequestHandlerFunction(String staticRoot) {
		this(staticRoot, true, DEFAULT_CACHE_EXPIRE);
	}

	public StaticRequestHandlerFunction(String staticRoot, boolean classpath) {
		this(staticRoot, classpath, DEFAULT_CACHE_EXPIRE);
	}

	public StaticRequestHandlerFunction(String staticRoot, boolean classpath, int cacheDuration) {
		this.classpath = classpath;
		if (classpath) {
			// 使用classpath里面的资源
			this.staticRoot = HttpUtils.normalize(staticRoot);
		} else {
			// 使用文件系统的资源
	...
	public void sendResource(HttpContext ctx, String path) {
		InputStream is = null;
		byte[] bytes = null;
		try {
	...
	public void sendFile(HttpContext ctx, String path, String ifModifiedSince) {
