...
public class HttpRequest {
	...
	public HttpRequest(FullHttpRequest req) {
		...
		this.relativeUri = HttpUtils.normalize(path());
	}
	...
	public <T> List<T> jsonArrayForm(Class<T> clazz) {
		byte[] content = new byte[req.content().readableBytes()];
		req.content().readBytes(content);
		return JSON.parseArray(new String(content, server.HttpUtils.UTF8), clazz);
	}

	public String dataForm() {
		byte[] content = new byte[req.content().readableBytes()];
		req.content().readBytes(content);
		return new String(content, server.HttpUtils.UTF8);
	}
	...
	public HttpRequest attr(String name, Object value) {
		attributes.put(name, value);
		return this;
	}

	public HttpRequest filter(IRequestFilter filter) {
